...
elevatorsMap = [...Array(nbFloors)].map((i) => exitPos);
...
for (let i = 0; i < nbElevators; i++) {
  ...
  elevatorsMap[elevatorFloor] = elevatorPos;
}
console.error(elevatorsMap);
// game loop
while (true) {
  var inputs = readline().split(" ");
  ...
  if (
    (clonePos < elevatorsMap[cloneFloor] && direction == "LEFT") ||
    (clonePos > elevatorsMap[cloneFloor] && direction == "RIGHT")
  )
    console.log("BLOCK");
